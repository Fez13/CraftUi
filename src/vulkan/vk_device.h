...
public:
  ...
  /*
    @brief Creates a one time use command buffer, it will be deleted after being submitted.
  */
  VkCommandBuffer
  ...
  /*
    @brief Creates one or more one time use command buffer, they will be deleted after being submitted.
  */
  ...
  /*
    ...
    @warning "pool_name" must be the same used to create the command buffer, and count must not exceed the count of buffers.
  */
...
public:
  ...
  */
  inline bool contains(const std::string &name){return m_devices.contains(name);}
